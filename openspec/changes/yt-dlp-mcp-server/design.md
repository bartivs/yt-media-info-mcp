...
The Node container calls `http://yt-dlp-service:8000/<endpoint>`. No host Docker socket mount is required. This removes a security surface and a dependency on docker-cli in the Node image. The Python service is `expose`-only (not `ports`) — internal to the compose network.
...
- [Auth API key in committed `env.example` defaults] → Mitigation: default `YTDLP_API_KEY` is empty (auth disabled); document that users should copy `env.example` to `.env` and set the key. Never log the key.
